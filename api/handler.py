...
def configure_entrez_and_patch():
    ...
    try:
        ...
        if not os.path.exists(custom_dtd_dir):
            # Ensure mode is compatible with older original_makedirs if it doesn't support exist_ok
            if not os.path.exists(custom_dtd_dir):
                 original_makedirs(custom_dtd_dir, mode=0o755)
...
def _call_entrez_with_retry(action_description, action_func, max_retries=3, base_delay_seconds=0.34):
    ...
    for attempt in range(max_retries):
        ...
        except Entrez.EntrezError as e: # Assuming Entrez is Bio.Entrez
            last_exception = e
...
def microbe_search_endpoint(): # Renamed to avoid confusion with any potential internal 'microbe_search' var/func
    ...
    try:
        ...
        ids_to_fetch_details_for = id_list[:20] 
        for tax_id_str in ids_to_fetch_details_for:
            try:
                details = _fetch_single_microbe_details(tax_id_str) 
...
def parse_biosample_xml(xml_string):
    ...
    try:
        root = ET.fromstring(xml_string) # Expects a single BioSample element as root for this parser
        # If xml_string is a full BioSampleSet, this needs adjustment or the input needs to be pre-processed
        # Assuming xml_string is for a single BioSample based on usage in _fetch_single_microbe_details
        
        # If the root is BioSampleSet, iterate through BioSample child elements
        # For now, assuming the passed xml_string is already a single BioSample string like '<BioSample>...</BioSample>'
        
        # If the function is meant to parse a string that might be <BioSampleSet><BioSample>...</BioSample></BioSampleSet>
        # then it needs to find 'BioSample' first. Let's assume it's a single BioSample.
        
        # The original code inside _fetch_single_microbe_details passes ET.tostring(sample_node), 
        # so `sample_node` IS the BioSample element. Thus, `root` here is the BioSample element.
        
        sample_data['id'] = root.get('id')
        sample_data['accession'] = root.get('accession')
        for desc_element in root.findall('./Description/*'):
            if desc_element.tag == 'Title':
                sample_data['title'] = desc_element.text
            if desc_element.tag == 'CommentParagraph' or desc_element.tag == 'Paragraph': 
        ...
        for attribute in root.findall('./Attributes/Attribute'):
            ...
            display_name = attribute.get('display_name', attr_name) 
            value = attribute.text
            sample_data["attributes"].append({"name": display_name, "value": value})
            if attr_name and "gram" in attr_name.lower(): 
            ...
            if display_name and "gram" in display_name.lower(): 
                if value and "positive" in value.lower(): gram_stain_found = "Gram-positive"
                elif value and "negative" in value.lower(): gram_stain_found = "Gram-negative"
            ...
            for attr_dict in sample_data["attributes"]:
                ...
                if val:
                    if "gram-positive" in val.lower() or "gram positive" in val.lower():
                        ...
                        break
        if gram_stain_found:
            sample_data["gram_stain_biosample"] = gram_stain_found
    except ET.ParseError as pe:
        print(f"[BIOSAMPLE_PARSE_ERR] XML Parse Error in parse_biosample_xml: {pe} for xml: {xml_string[:200]}") # Log part of xml
    except Exception as e:
        print(f"[BIOSAMPLE_PARSE_ERR] Unexpected error parsing BioSample XML in parse_biosample_xml: {e}")
    return sample_data
...
def _fetch_single_microbe_details(tax_id):
    ...
    oxygen_requirement_general = None 
        ...
        for name_info in tax_record["OtherNames"]["Name"]:
            if isinstance(name_info, dict) and name_info.get('ClassCDE') == 'comment':
                ...
                if not oxygen_requirement_general:
                    ...
                    elif "facultative" in comment_text: oxygen_requirement_general = "Facultative" 
    if gram_stain_general:
        result["gram_stain_taxonomy_comment"] = gram_stain_general
    ...
    if nuccore_links and nuccore_links[0].get("LinkSetDb"):
        ...
        if nuccore_ids:
            def action_summarize_nuccore():
                handle = Entrez.esummary(db="nuccore", id=",".join(nuccore_ids[:20])) 
        ...
        if biosample_ids:
            ...
            if biosample_xml_data:
                try:
                    ...
                    # Ensure the XML string is wrapped in a single root element if it's not already.
                    # Entrez efetch for multiple BioSample IDs returns a BioSampleSet.
                    if not biosample_xml_data.strip().startswith('<BioSampleSet>'):
                        # This case might happen if only one ID was fetched and it returned a single <BioSample>
                        # or if the XML is malformed. For safety, wrap if not clearly a BioSampleSet.
                        # However, efetch with multiple IDs should return BioSampleSet.
                        # If it's a single BioSample, ET.fromstring will work, but findall('BioSample') on it might not be what's intended.
                        # The original code structure implies we expect a BioSampleSet if multiple IDs.
                        # For a single ID, it might return just <BioSample>.
                        # Let's assume `parsed_biosamples_root` is the correct root (e.g. BioSampleSet).
                        # If it's a single <BioSample>, then findall('BioSample') might need adjustment or
                        # the loop `for sample_node in parsed_biosamples_root.findall('BioSample')`
                        # should become conditional.
                        # For now, sticking to the original logic of wrapping if not <BioSampleSet>.
                    ...
                    if not root_node_str:
                        ...
                        parsed_biosamples_root = ET.fromstring(root_node_str) # This is BioSampleSet
                        for sample_node in parsed_biosamples_root.findall('BioSample'): # Iterate through each <BioSample>
                            sample_xml_str_for_parser = ET.tostring(sample_node, encoding='unicode')
                            parsed_sample_data = parse_biosample_xml(sample_xml_str_for_parser) # Parse individual BioSample
                            
                            if parsed_sample_data: 
                                # Oxygen requirement logic for the current sample
        ...
        for sample in result["biosample_info"]:
            if sample.get("attributes"):
                for attr in sample["attributes"]:
                    if attr.get("name", "").lower() in ["isolation source", "host", "geo_loc_name"]: 
                        primary_isolation_source = attr.get("value", "N/A")
                        break
                if primary_isolation_source != "N/A":
                    break
        if primary_isolation_source == "N/A" and result["biosample_info"][0].get("attributes"): 
...
@app.route('/api/microbes/details', methods=['POST', 'OPTIONS']) 
def microbes_details_batch():
    ...
    for tax_id in tax_ids:
        try:
            details = _fetch_single_microbe_details(str(tax_id)) 
